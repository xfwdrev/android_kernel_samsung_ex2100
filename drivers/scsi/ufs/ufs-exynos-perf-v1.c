...
static int __ctrl_dvfs(struct ufs_perf *perf, enum ctrl_op op)
{
	...
	if (op == CTRL_OP_UP) {
		if (perf->val_pm_qos_cluster0 &&
		    freq_qos_request_active(&perf->pm_qos_cluster0) &&
		...
		if (perf->val_pm_qos_cluster1 &&
		    freq_qos_request_active(&perf->pm_qos_cluster1) &&
		...
		if (perf->val_pm_qos_cluster2 &&
		    freq_qos_request_active(&perf->pm_qos_cluster2) &&
...
{
	trace_ufs_perf_lock("wb", op);

	if (op == CTRL_OP_UP)
		return ufshcd_wb_ctrl(perf->hba, true);
	else if (op == CTRL_OP_DOWN)
		return ufshcd_wb_ctrl(perf->hba, false);
				...
				enum __traffic traffic, s64 time)
{
	...
	if (traffic == TRAFFIC_HIGH) {
		if (stat->freq_state == FREQ_RARE) {
			...
			pr_warn_once("%s: unexpected freq state %d\n",
				     __func__, stat->freq_state);
			stat->freq_state = FREQ_REACH;
			transit = true;
		}
	...
	if (traffic == TRAFFIC_HIGH) {
		if (stat->hat_state == HAT_RARE) {
			...
			*/
			...
			pr_warn_once("%s: unexpected hat state %d\n",
				     __func__, stat->hat_state);
			stat->hat_state = HAT_REACH;
			transit = true;
		}
	} else if (traffic == TRAFFIC_LOW) {
...
int ufs_perf_init_v1(struct ufs_perf *perf)
{
	...
	for (index = 0; index < (int)nr_cpu_ids; index++) {
		const struct cpumask *mask_cur = topology_core_cpumask(index);
		if (cluster >= ARRAY_SIZE(masks_per_cluster))
			break;
...
void ufs_perf_exit_v1(struct ufs_perf *perf)
{
	...
	del_timer_sync(&stat->reset_timer);
