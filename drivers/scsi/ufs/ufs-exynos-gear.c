...
/* Keep in sync with ufshcd.c/ufs-exynos.c local state values. */
enum {
	UFSHCD_STATE_RESET,
	UFSHCD_STATE_ERROR,
...
static int exynos_gear_call_cal(struct exynos_ufs *ufs, cal_if_func func)
{
...
int ufs_gear_change(struct ufs_hba *hba, bool en)
{
	...
	res = exynos_gear_call_cal(ufs, ufs_cal_pre_pmc);
	if (res) {
	...
	/* post pmc */
	res = exynos_gear_call_cal(ufs, ufs_cal_post_pmc);
	if (res) {
...
void ufs_gear_scale_exit(struct ufs_perf *perf)
{
	...
	unregister_reboot_notifier(&stat->reboot_nb);
}
